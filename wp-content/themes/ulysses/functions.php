<?php
if( !function_exists('ulysses_get_the_ID') ) :

	function ulysses_get_the_ID() {
 ...
 */
function ulysses_dynamic_menu_items( $menu_items ) {
    ...
    foreach ( $menu_items as $menu_item ) {
        ...
        if ( '#' === $url && !is_front_page() ) {
			$menu_item->url = trailingslashit( get_bloginfo('url') ) . $menu_item->url;
        }
    }

    return $menu_items;
}
add_filter( 'wp_nav_menu_objects', 'ulysses_dynamic_menu_items' );
